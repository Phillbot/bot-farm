...
import { environment, getReactClickerConfigOrThrow } from '@config/environment';
...
export const databaseModule = new ContainerModule((bind) => {
  ...
  bind<string>(NbuBotPostgresConnectUrl.$).toConstantValue(environment.nbu.postgresUrl);

  if (environment.nbu.postgresPort) {
    bind<number>(NbuBotPostgresPort.$).toConstantValue(environment.nbu.postgresPort);
  }
